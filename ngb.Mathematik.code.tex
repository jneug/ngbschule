...
\NewDocumentCommand \tu {o o o} {
	\qquad\vert \IfNoValueTF{#1}{\,\text{TU}}{#1}%
	\IfNoValueF{#2}{\quad\vert #2}%
...
\newcommand{\Linienbreite}[1][1.2pt]{\tikzset{line style/.style={line width=#1}}}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                Objekte                %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\NewDocumentCommand \Text { O{} r() m } {\tkzText[fill=white,#1](#2){#3}}

% Punkte %
\newcommand{\Ursprung}[1][O]{\tkzDefPoint(0,0){#1}}
\NewDocumentCommand \Koordinate { r() m }{\tkzDefPoint(#1){#2}}
\NewDocumentCommand \Koordinaten {m}{\tkzDefPoints{#1}}

\NewDocumentCommand \Punkt {s O{} r() m}{
	\tkzDefPoint(#3){#4}\tkzDrawPoint[shape=cross out,size=10,#2](#4)
	\IfBooleanT {#1}
		{ \PunktBeschriften(#4){#4} }
}

\NewDocumentCommand \PunktMarkieren { r() }{\tkzPointShowCoord(#1)}
\NewDocumentCommand \PunktBeschriften { O{} r() m }{\tkzLabelPoint[#1](#2){#3}}

% TODO: Spiegelpuntke auch shape=cross out setzen, oder als Kreis lassen?
\NewDocumentCommand \Spiegelpunkt {s O{} r() r() m}{
	\IfBooleanTF {#1}
...
\NewDocumentCommand \Drehpunkt {s O{} r() r() m m}{
	\IfBooleanTF {#1}
...
\NewDocumentCommand \Strecke { O{} r() }{\tkzDrawSegment[#1](#2)}
\NewDocumentCommand \Strecken { O{} r() }{\tkzDrawSegments[#1](#2)}
\NewDocumentCommand \StreckeBeschriften { O{} r() m }{\tkzLabelSegment[#1](#2){#3}}

\NewDocumentCommand \Laenge { O{} r() O{} m } {\tkzDrawSegment[line width=1pt,|-|,#1](#2)\tkzLabelSegment[#3](#2){#4}}

\NewDocumentCommand \Gerade { O{} r() }{\tkzDrawLine[add=10 and 10,#1](#2)}
\NewDocumentCommand \GeradeBeschriften { O{} r() m }{\tkzLabelSegment[#1](#2){#3}}

\NewDocumentCommand \Strahl { O{} r() }{\tkzDrawLine[add=0 and 10,#1](#2)}

\NewDocumentCommand \Vektor { O{} r() }{\tkzDrawSegment[>=latex,->,thick,#1](#2)}

% Formen und Polygone %
%\NewDocumentCommand \Polygon { O{} r() }{\tkzDrawPolySeg[#1](#2)}
\NewDocumentCommand \Polygon { O{} r() }{\tkzDrawPolygon[#1](#2)}

% Transformationen %
\NewDocumentCommand \Spiegeln { s r() r() O{'} }{%
	\IfBooleanTF {#1} {%
		\foreach \p in {#2}{%
			\Spiegelpunkt*(\p)(#3){\p#4}
		}%
	}{%
		\foreach \p in {#2}{%
			\Spiegelpunkt(\p)(#3){\p#4}
